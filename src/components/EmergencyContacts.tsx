...
const EmergencyContacts: React.FC<EmergencyContactsProps> = ({ open, onClose }) => {
  ...
  const handleAdd = () => {
    ...
    addEmergencyContact({ name: name.trim(), phone: phone.trim(), relationship: relationship.trim() || 'Other' });
    setName('');
    setPhone('');
    setRelationship('');
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[9998] flex items-end sm:items-center justify-center bg-foreground/30 backdrop-blur-sm"
          onClick={onClose}
        >
          <motion.div
            initial={{ y: 100 }}
            ...
            className="bg-card rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-md max-h-[85vh] overflow-y-auto soft-shadow"
          >
            ...
            <div className="space-y-2">
              {user?.emergencyContacts.length === 0 && (
                <p className="text-muted-foreground text-sm text-center py-4">No emergency contacts yet</p>
              )}
